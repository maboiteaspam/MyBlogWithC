...
class Controllers
{
    ...
    public function home()
    {
        return function (Application $app, Request $request) {
            ...
            $entriesTag = new RepositoryGhoster ($app[$this->entryRepo]);
            /* @var $entriesTag \C\BlogData\Eloquent\EntryRepository */

            $entries = new RepositoryGhoster ($app[$this->entryRepo]);
            $entries->setTag($entriesTag->lastUpdateDate()->first());
            /* @var $entries \C\BlogData\Eloquent\EntryRepository */

            $entryCount = new RepositoryGhoster ($app[$this->entryRepo]);
            /* @var $entryCount \C\BlogData\Eloquent\EntryRepository */


            $commentsTag = new RepositoryGhoster ($app[$this->commentRepo]);
            /* @var $commentsTag \C\BlogData\Eloquent\CommentRepository */

            $comments = new RepositoryGhoster ($app[$this->commentRepo]);
            $comments->setTag($commentsTag->lastUpdateDate()->first());
    ...
    public function detail($postCommentUrl, $postCommentUrlParams=[])
    {
        return
            function (Application $app, Request $request, $id)
            use ($postCommentUrl, $postCommentUrlParams) {

                $entry = new RepositoryGhoster($app[$this->entryRepo]);
                /* @var $entry  \C\BlogData\Eloquent\EntryRepository */


                $commentsByEntryTag = new RepositoryGhoster($app[$this->commentRepo]);
                /* @var $commentsByEntryTag \C\BlogData\Eloquent\CommentRepository */

                $commentsByEntry = new RepositoryGhoster($app[$this->commentRepo]);
                $commentsByEntry
                    ->setTag($commentsByEntryTag->lastUpdatedByEntryId($id)->first());
                /* @var $commentsByEntry \C\BlogData\Eloquent\CommentRepository */


                $mostRecentCommentsTag = new RepositoryGhoster($app[$this->commentRepo]);
                /* @var $mostRecentCommentsTag \C\BlogData\Eloquent\CommentRepository */

                $mostRecentComments = new RepositoryGhoster($app[$this->commentRepo]);
                $mostRecentComments
                    ->setTag($mostRecentCommentsTag->lastUpdatedByEntryId($id)->first());
                ...
                $params = [];
                foreach ($postCommentUrlParams as $p) {
                    $params[$p] = $request->attributes->get($p);
                }
                Transforms::transform()
                    ->setLayout($app['layout'])
                    ->setGenerator($app['url_generator'])
                    ->updateFormRoute('blog_form_comments',
                        'form', $postCommentUrl, $params
                    );
